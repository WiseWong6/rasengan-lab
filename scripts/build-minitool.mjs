...
const require=createRequire(import.meta.url),P=require('../src/physics.js'),A=require('../src/animations.js');
...
for(const name of ['physics','evolution','animations','app']){
 const result=await transform(readFileSync(new URL('src/'+name+'.js',root),'utf8'),{target,minify:true,legalComments:'none'});
...
await build({absWorkingDir:root.pathname,entryPoints:['src/rendering/hero.ts'],outfile:new URL('assets/hero-renderer.js',out).pathname,bundle:true,format:'iife',globalName:'RasenganRenderer',target,minify:true,legalComments:'none'});
...
html=html.replace('<script src="src/physics.js">','<script src="./assets/seeds.js"></script><script src="./assets/compat.js"></script><script src="./assets/physics.js">');
for(const name of ['evolution','animations','hero-renderer','app']){
 const folder=name==='hero-renderer'?'assets':'src';
 html=html.replace('src="'+folder+'/'+name+'.js"','src="./assets/'+name+'.js"');
}
