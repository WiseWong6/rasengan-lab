...
export function create(canvas:HTMLCanvasElement,options:{size?:number;maxTriangles?:number;samples?:number;airflow?:number;secondary?:number}={}){
  const renderer=new LightRenderer(canvas,options);
  const steps=options.samples??80,secondarySteps=Math.min(64,steps);
  return {
    draw(P:any,cloud:any[],state:any,trail:number){
      ...
      for(let n=0;n<count;n++){
        ...
        const points=Array.from({length:steps+1},(_,j)=>project(sample(index,length*(1-j/steps)),camera));
        fibers.push({points,width:n%9===0?3.8:2.4,opacity:power*reveal*(n%9===0?.98:.88),accent:n%9===0,energy:1,taper:true});
      }
      for(let n=0;n<Math.min(options.secondary??192,count/2);n++){
        const index=n*137%count,phi=n*2.399963;
        const points=Array.from({length:secondarySteps+1},(_,j)=>project(rotate(sample(index,trail*(1-j/secondarySteps)),0,phi),camera));
        fibers.push({points,width:1.7,opacity:power*build*.34,accent:false,energy:1,taper:true});
      }
      paintAirflow(renderer,camera,13+state.time,build*power,(options.airflow??10)*build);
      ...
      if(state.animation==='lightning'){
        ...
        for(let n=0;n<30;n++){
          ...
          const points=Array.from({length:secondarySteps+1},(_,j)=>project(sample(index,length*(1-start-(head-start)*j/secondarySteps)),camera));
